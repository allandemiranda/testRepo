...
public class ForexProgram implements Runnable {
  ...
  public void run() {
    ...
    this.setTimeValue(Arrays.stream(output.stream().filter(s -> s.contains(STARTED_FOREX_APPLICATION_IN)).findFirst().orElse(":ERROR").split(":")).collect(Collectors.toCollection(LinkedList::new)).getLast());
  }
}
